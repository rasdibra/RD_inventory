...
-- RD INVENTORY HARD FIX
-- Open dpclothing wheel from RD_inventory CLOTHES button.
-- It does not toggle; it always opens and keeps it alive shortly so RD NUI closing cannot cancel it.
RegisterNetEvent('dpc:RDInventoryOpen')
AddEventHandler('dpc:RDInventoryOpen', function()
    local ped = PlayerPedId()
    if Check and not Check(ped) then return end

    MenuOpened = true
    if SoundPlay then SoundPlay("Open") end
    if Config and Config.GUI and Config.GUI.Position then
        SetCursorLocation(Config.GUI.Position.x, Config.GUI.Position.y)
    end

    CreateThread(function()
        local untilTime = GetGameTimer() + 900
...
RegisterCommand('rdopendpclothing', function()
